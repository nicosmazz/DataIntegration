...
public class SparqlQuery {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			try (QueryExecution qexec = QueryExecutionFactory.sparqlService(service, query);) {
				...
				for (; resultSet.hasNext();) {
					...
					//faccio un check sull'oggetto, al fine di verificare che sia una risorsa oppure un Literal
					if (solution.get("o") instanceof Resource) {
						...
						if (model.containsResource(solution.getResource("s"))) {
							model.getResource(solution.getResource("s").toString()).addProperty(
									...
									//se è una risorsa il valore della propietà viena definito creando una nuova risorsa
									model.createResource(solution.getResource("o").toString()));
						} else {
							//se la risorsa non è presente nel modello la creo
							model.createResource(solution.getResource("s")).addProperty(
									model.createProperty(namespace, localName),
									model.createResource(solution.getResource("o").toString()));
						}
					} else {
						//se l'object estratto è un Literal il valore della propietà viene definito per mezzo di una Stringa
						if (model.containsResource(solution.getResource("s"))) {
							int length = solution.get("o").toString().length();
							model.getResource(solution.getResource("s").toString()).addProperty(
									model.createProperty(namespace, localName), solution.get("o").toString().substring(0, length-3));
						} else {
							int length = solution.get("o").toString().length();
							model.createResource(solution.getResource("s")).addProperty(
									model.createProperty(namespace, localName), solution.get("o").toString().substring(0, length - 3));
	...
	/*
	 * il metodo è analogo al precedente con la differenza che gestisce il caso
	 * in cui il Subject fornito per la ricerca sia un literal
	 */
	public static Model QuerySparql(String subject, boolean literal) {
		Model model = ModelFactory.createDefaultModel();
			...
			String queryString;
			if(literal) {
				queryString = "Select ?s ?p ?o where" + "{" + "{" + "?s ?p ?o. " + "FILTER (?s = \""
						+ subject + "\"@it)." + "}" + "UNION" + "{" + "?s ?p ?o. " + "FILTER (?o = \""
						+ subject + "\"@it)." + "}" + "} LIMIT 20";
			} else {
				queryString = "Select ?s ?p ?o where" + "{" + "{" + "?s ?p ?o. " + "FILTER (?s = <"
						+ subject + ">)." + "}" + "UNION" + "{" + "?s ?p ?o. " + "FILTER (?o = <"
						+ subject + ">)." + "}" + "} LIMIT 20";
			}
			Query query = QueryFactory.create(queryString);

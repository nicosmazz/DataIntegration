...
public class Path {
	...
	public static String writePath(ArrayList<Path> paths) {
		...
		for(int i=1; i<paths.size(); i++) {
			if(paths.get(i).object == paths.get(i-1).object) {
				percorso = percorso.concat(" <- " + "(p) " + paths.get(i).predicate + " <- " + "(s) " + paths.get(i).subject);
			} else if(paths.get(i).subject == paths.get(i-1).object) {
